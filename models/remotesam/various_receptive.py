...
def _conv_bn(
    input_channel, output_channel, kernel_size=3, padding=1, stride=1, groups=1
):
    res = nn.Sequential()
    res.add_module(
        "conv",
...
class RepBlock(nn.Module):
    def __init__(
        self, input_channel, output_channel, kernel_size=3, groups=1, stride=1
    ):
        super().__init__()
    ...
    def forward(self, inputs):
        if self.brb_identity is None:
            identity_out = 0
        else:
            identity_out = self.brb_identity(inputs)
        out1 = self.activation(
            self.brb_1x1(inputs) + self.brb_3x3(inputs) + identity_out
        )

        if self.brb_identity_2 is None:
            identity_out_2 = 0
        else:
