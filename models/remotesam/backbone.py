...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l_feats, l_mask):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww, l_feats, l_mask)
...
class MMBasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, l_feats, l_mask):
        ...
        x = (
            x
            + (self.res_gate(x_residual) * x_residual)
...
class PWAM(nn.Module):
    ...
    def forward(self, x, l_feats, l_mask):
        ...
        lang = self.image_lang_att(x, l_feats, l_mask)  # (B, H*W, dim)
...
class SpatialImageLanguageAttention(nn.Module):
    ...
    def forward(self, x, l_feats, l_mask):
        ...
        key = self.f_key(l_feats)  # (B, key_channels, N_l)
        value = self.f_value(l_feats)  # (B, self.value_channels, N_l)
        key = key * l_mask  # (B, key_channels, N_l)
        value = value * l_mask  # (B, self.value_channels, N_l)
        n_l = value.size(-1)
        query = query.reshape(
            B, HW, self.num_heads, self.key_channels // self.num_heads
        ).permute(0, 2, 1, 3)
        # (b, num_heads, H*W, self.key_channels//self.num_heads)
        key = key.reshape(B, self.num_heads, self.key_channels // self.num_heads, n_l)
        # (b, num_heads, self.key_channels//self.num_heads, n_l)
        value = value.reshape(
            B, self.num_heads, self.value_channels // self.num_heads, n_l
        )
        ...
        out = torch.matmul(
            sim_map, value.permute(0, 1, 3, 2)
        )  # (B, num_heads, H*W, self.value_channels//num_heads)
        out = (
            out.permute(0, 2, 1, 3).contiguous().reshape(B, HW, self.value_channels)
