...
class Linear_BN(torch.nn.Sequential):
    ...
    @torch.no_grad()
    def fuse(self):
        layer, bn = self._modules.values()
        w = bn.weight / (bn.running_var + bn.eps) ** 0.5
        w = layer.weight * w[:, None]
...
class CrossScaleAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # [B, C, H, W] -> [B, C, H*W]
        x0, x1, x2 = (
            x0.view(x0.shape[0], x0.shape[1], -1),
            x1.view(x1.shape[0], x1.shape[1], -1),
            x2.view(x2.shape[0], x2.shape[1], -1),
        )
...
class CIM(nn.Module):
    def __init__(self, dim, num_layers=1, channels=[128, 256, 512, 1024], downsample=1):
        ...
        self.block = nn.ModuleList(
            [CIMBlock(self.hidden_dim, channels) for _ in range(num_layers)]
        )
        self.bn = nn.BatchNorm2d(self.hidden_dim)
        self.fusion = nn.ModuleList(
            [ScaleAwareGate(channels[i], channels[i]) for i in range(len(channels))]
        )
