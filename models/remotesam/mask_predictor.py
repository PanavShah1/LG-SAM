...
class SimpleDecoding(nn.Module):
    def __init__(self, c4_dims, factor=2):
        ...
        self.conv1_4 = nn.Conv2d(
            c4_size + c3_size, hidden_size, 3, padding=1, bias=False
        )
        ...
        self.conv1_3 = nn.Conv2d(
            hidden_size + c2_size, hidden_size, 3, padding=1, bias=False
        )
        ...
        self.conv1_2 = nn.Conv2d(
            hidden_size + c1_size, hidden_size, 3, padding=1, bias=False
        )
    ...
    def forward(self, x_c4, x_c3, x_c2, x_c1):
        ...
        if x_c4.size(-2) < x_c3.size(-2) or x_c4.size(-1) < x_c3.size(-1):
            x_c4 = F.interpolate(
                input=x_c4, scale_factor=2, mode="bilinear", align_corners=True
            )
        ...
        if x.size(-2) < x_c2.size(-2) or x.size(-1) < x_c2.size(-1):
            x = F.interpolate(
                input=x, scale_factor=2, mode="bilinear", align_corners=True
            )
