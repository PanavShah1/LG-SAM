...
def _get_rotation_matrix(thetas):
        ...
        (
            torch.cat(
                (
                    ...
                    torch.zeros(1, 2, bs * g, device=device),
                    1 - c,
                    torch.zeros(1, 5, bs * g, device=device),
                ),
                dim=1,
            ),
            torch.cat(
            ...
            torch.cat(
                (
                    ...
                    1 - c,
                    c,
                    torch.zeros(1, 6, bs * g, device=device),
                ),
                dim=1,
            ),
            torch.cat(
    ...
    rot_mat = (
        mask * rot_mat_positive + (1 - mask) * rot_mat_negative
    )  # shape = [k*k, k*k, bs*g]
    rot_mat = rot_mat.permute(2, 0, 1)  # shape = [bs*g, k*k, k*k]
    rot_mat = rot_mat.reshape(
        bs, g, rot_mat.shape[1], rot_mat.shape[2]
    )  # shape = [bs, g, k*k, k*k]
    return rot_mat
