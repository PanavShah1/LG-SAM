...
from prompts import EXTRACT_NUMERIC_QUESTION_TYPE_PROMPT
...
def batch_generate(batch_items):
    ...
    is_counting_question_res = qwen3_vl_8b.generate(
        images,
        [
            EXTRACT_NUMERIC_QUESTION_TYPE_PROMPT.format(question=question)
            for question in prompts
        ],
    )
    answers = []
    for is_counting_question, image, prompt in zip(
        is_counting_question_res, images, prompts
    ):
        is_counting_question = (
            is_counting_question.strip().split("\n")[-1].strip().lower()
        )
        if "yes" in is_counting_question or "true" in is_counting_question:
            bboxes, _ = remotesam.detect(image, prompt)
            answers.append(str(len(bboxes)))
        else:
            answers.append(earthmind.answer(image, f"Numeric question: {prompt}"))
    return answers
