...
from pipelines.falcon import FalconPipeline
...
def process_annotation_batch(
    ...
    for ann_id in annotation_ids:
        ...
        for obj_idx, obj in enumerate(ann_data.get("objects", [])):
            ...
            if gt_obj_corners is None or len(gt_obj_corners) != 8:
                continue
            ...
            batch_metadata.append(
                (ann_id, [gt_obj_corners], referring_sentence, obj_idx)
            )
            ann_processed = True
...
def test_pipeline_on_vrsbench(
    ...
    if use_annotations_file:
        ...
        annotation_objects = sorted(annotations_path.glob("P*.json"))
    ...
    for ann_filename, image_results in sorted(results_dict.items()):
        ...
        if len(image_results) > 0:
            print(
                ...
                f"mean IoU: {mean_iou:.4f}, score: {score:.4f} (pred: {np.sum(results_arr[:, 2])}, gt: {np.sum(results_arr[:, 1])})"
            )
        else:
            print(
                f"{ann_filename}: {len(image_results)} objects matched, "
